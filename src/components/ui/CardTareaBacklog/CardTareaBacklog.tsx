...
export const CardTareaBacklog: FC<ICardTareaBacklog> = ({
  ...
  const enviarTareaASprint = async () => {
    ...
    try {
      // Actualizamos el sprint en el backend
      await putTareaSprint(sprintActualizada);

      // Asegurarnos de que la tarea tenga un ID antes de eliminarla del backlog
      if (tarea.id) {
        await eliminarTareaBacklog(tarea.id); // Eliminar la tarea del backend
        eliminarTareaById(tarea.id); // Eliminar tarea del store
      } else {
        console.error(
